...
export type ImagePoolItem = { src: string; alt?: string } | string;
...
type BuiltItem = Coords & ImageItem;
...
function buildItems(pool: ImagePoolItem[], seg: number): BuiltItem[] {
    const xCols = Array.from({ length: seg }, (_, i) => -37 + i * 2);
    const evenYs = [-4, -2, 0, 2, 4];
    const oddYs = [-3, -1, 1, 3, 5];

    const coords: Coords[] = xCols.flatMap((x, c) => {
    ...
    const usedImages: ImageItem[] = Array.from({ length: totalSlots }, (_, i) => normalizedImages[i % normalizedImages.length]);

    // Attempt to de-dupe adjacent images
...
export default function DomeGallery({
    ...
    const originalTilePositionRef = useRef<PositionRect | null>(null);
    ...
    useEffect(() => {
        ...
        const ro = new ResizeObserver(entries => {
            ...
            if (enlargedOverlay instanceof HTMLElement && frameRef.current && mainRef.current) {
